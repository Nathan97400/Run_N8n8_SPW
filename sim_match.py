...
for a in lbc:
    ...
    ad_text_for_cc = (ad_text_base + ' ' + model_hint).strip()  # for CC/year extraction
    ad_text_used = (title + ' ' + model_hint).strip()  # FIX: title only (no desc) for matching
    ...
    if best_p is None:
        ...
        if len(fail_samples) < 5:
            fail_samples.append(('no_candidate', title, model_hint, bk_ann, ann_cc))
    elif best_score >= MIN_ACCEPT:
        matched += 1
    else:
        below_thresh += 1
        if len(fail_samples) < 5:
            fail_samples.append(('below_thresh', title, model_hint, bk_ann, ann_cc, round(best_score,3)))

total = len(lbc)
print(f'Total: {total}')
print(f'Matched: {matched} ({round(matched/total*100)}%)')
print(f'No brand in index: {no_brand}')
print(f'No candidate (dom/cov gate): {no_model}')
