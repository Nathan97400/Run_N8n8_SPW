"""
Simulation with all fixes applied:
1. O→0 normalization
2. CC sanity check (reject > 2500)
3. DOM proximity fix
4. Pass 3 without CC gate
5. Supplemental brands (VOGE, Indian, QJ Motor, etc.)
"""
...
def normalize(t):
    ...
    # FIX 1: letter O as zero in model numbers
...
# FIX 5: Load supplemental brands
with open('supplement_bdd.json','r',encoding='utf-8') as f:
    supplement=json.load(f)

all_bdd = worker_bdd + supplement
print(f'Worker BDD: {len(worker_bdd)}, Supplement: {len(supplement)}, Total: {len(all_bdd)}')

brand_index={}
for m in all_bdd:
    br=m.get('Marque','') or ''
    mo=m.get('Mod\u00e8le','') or m.get('Modele','') or ''
    if not br or not mo: continue
    bk=bkey(br)
    cc_raw=m.get('Cylindr\u00e9e (cc)') or m.get('Cylindree','')
    cc=round(float(cc_raw)) if cc_raw else 0
    ys_raw=m.get('Ann\u00e9e d\u00e9but') or m.get('Annee_debut')
    ye_raw=m.get('Ann\u00e9e fin') or m.get('Annee_fin')
...
brands_count = len(brand_index)
print(f'Brand index: {brands_count} brands')
...
for a in lbc:
    ...
    # FIX 4: 3 passes (with year+CC, without year but CC, without CC)
...
print(f'Gain vs baseline (88.7%): +{round((fail["matched"]-2207)/total*100,1)}%')
