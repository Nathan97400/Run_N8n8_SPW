"""
Diagnostic approfondi pour tous les no_candidate du real batch
"""
...
for a in lbc:
    ...
    if best_p is None:
        print(f"NO_CAND: {brand_str}: {title[:50]}")
        print(f"  model_hint={model_hint!r} ann_cc={ann_cc} ann_year={ann_year}")
        print(f"  ad_tok={ad_tok}")
        print(f"  ad_set={sorted(ad_set)}")
        # Show all candidates and why they fail
        cands = brand_index[bk]
        print(f"  Total {len(cands)} models for {bk}")
        # Find nearest matches by model name similarity
        top_by_tok = []
        for p in cands:
            ovlp = len(set(ad_tok) & set(p['ctok']))
            if ovlp > 0 or any(str(d) in ad_compact for d in p['doms'] if isinstance(d,int)):
                top_by_tok.append((ovlp, p))
        top_by_tok.sort(key=lambda x:-x[0])
        for ovlp, p in top_by_tok[:3]:
            c = max((cvg(ad_set,vat) for vat in p['vat']),default=0)
            dom_results = []
            for d in p['doms']:
                if isinstance(d,int):
                    ok = (ann_cc and abs(d-ann_cc)<=CC_BLOCK_DIFF) or d in ad_nums or str(d) in ad_comp
                    dom_results.append(f"{d}(int,ok={ok})")
                else:
                    d_clean=str(d).replace('-','').replace('_','')
                    ok = d_clean in ad_comp or d_clean in ad_compact
                    dom_results.append(f"{d!r}(str,ok={ok})")
            print(f"  Cand: {p['model_raw']!r} cc={p['cc']} ys={p['ys']} ye={p['ye']}")
            print(f"    vat={p['vat']} doms={dom_results}")
            print(f"    cvg={c:.2f}")
        print()
